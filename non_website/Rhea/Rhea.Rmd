...
Since budget is heavily right skewed, we log-transformed the data instead to enforce more normal distributions. We quantitatively tested the association of budget with categorical Bechdel score by regressing budget against the categories. 
...
```
Based on parameter estimate of -0.31003 with a p value of 0.01, it can be observed  that movies passing all 3 Bechdel dimensions have significantly lower budgets than movies failing every dimension of Bechdel test.
...
In the graph, profit appears to be heavily right skewed. Thus we log-transformed the data to enforce more normal distributions. Additionaly, before the logarithm is applied, 1 was added to the base value to prevent applying a logarithm to a 0 value. This 1 will be subtracted while calculating the results.
...
```
Based on the parameter estimates, it can be observed that movies that pass all dimensions of the bechdel test have lower profits than movies that fail all 3 dimensions of the bechdel test, adjusting for budget. However since the p-value is 0.7 > 0.05, the difference is statistically insignificant
...
movies that pass all the dimensions of Bechdel test have 0.344 point lower IMDB rating than mobies that fail all the dimensions of a Bechdel test.
