...
table = bind_cols(less2_df, notalk_df, talkmen_df, dubious_df, pass_df, .name_repair = "unique")

table %>% 
  plot_ly(x = ~decade_code...1, y = ~n...3, type = "bar", name = "Critical",
          marker = list(color = "3f4585ff")) %>% 
  add_trace(y = ~n...3, name = "", marker = list(color = "22958bff")) %>% 
  add_trace(y = ~n...6, name = "") %>% 
